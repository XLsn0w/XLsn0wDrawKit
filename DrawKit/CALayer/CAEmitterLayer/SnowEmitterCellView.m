
#import "SnowEmitterCellView.h"

@implementation SnowEmitterCellView

- (instancetype)initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (self) {
        [self initLayer];
    }
    return self;
}

- (void)initLayer {
    self.emitterLayer.emitterShape    = kCAEmitterLayerLine;                            // 直线粒子发射器
    self.emitterLayer.emitterMode     = kCAEmitterLayerSurface;                         // 边框包含下的区域进行抛洒
    self.emitterLayer.emitterSize     = self.frame.size;                                // 发射区域
    self.emitterLayer.emitterPosition = CGPointMake(self.bounds.size.width / 2.f, - 5); // 发射中心点位置
}

///CAEmitterLayer的创建主要包括两部分：CAEmitterLayer、CAEmitterCell，
///然后将其关联起来就可以了CAEmitterLayer.emitterCells = @[CAEmitterCell]
- (void)show {
    
    // 创建雪花类型的粒子
    CAEmitterCell *snowCell = [CAEmitterCell emitterCell];
    
    // 粒子的名字
    snowCell.name = @"snow";
    
    // 粒子参数的速度乘数因子
    snowCell.birthRate = (_birthRate > 0 ? _birthRate : 1.f);
    
    // 粒子生命周期
    snowCell.lifetime = (_lifetime > 0 ? _lifetime : 60);
    
    // 粒子速度
    snowCell.velocity = (_speed > 0 ? _speed : 10.f);
    
    // 粒子的速度范围
    snowCell.velocityRange = (_speedRange > 0 ? _speedRange : 10.f);
    
    // 粒子y方向的加速度分量(可以理解为重力)
    snowCell.yAcceleration = (_gravity != 0 ? _gravity : 2.f);
    
    // 每个发射的粒子的初始时候随机的角度
    snowCell.emissionRange = 0.5 * M_PI;
    
    // 粒子旋转角度
    snowCell.spinRange = 0.25 * M_PI;
    
    // 获取图片
    snowCell.contents = (id)_snowImage.CGImage;
    
    // 设置雪花形状的粒子的颜色
    snowCell.color = (_snowColor == nil ? [[UIColor whiteColor] CGColor] :_snowColor.CGColor);

    // 尺寸
    snowCell.scale = 0.5f;
    
    // 尺寸变化范围
    snowCell.scaleRange = 0.6f;

    // 添加粒子
    self.emitterLayer.emitterCells = @[snowCell];
    
    [UIView animateWithDuration:1.75f animations:^{
        self.alpha = 0.5f;
    }];
}

- (void)initWithType:(EmitterType)type {
    if (type == SnowEmitterType) {
        // 配置
        self.birthRate = 5.f;
        self.snowImage = [UIImage imageNamed:@"snow"];
        self.snowColor = [UIColor blackColor];
        self.lifetime  = 30.f;
        self.alpha     = 0.f;
        
        UIImageView *snowAlpha = [[UIImageView alloc] initWithFrame:self.bounds];
        snowAlpha.image        = [UIImage imageNamed:@"alpha"];
        self.maskView          = snowAlpha;
    }
}

- (void)hide {
    [UIView animateWithDuration:0.75 animations:^{
        self.alpha = 0.f;
    } completion:^(BOOL finished) {
        
    }];
}

@end
